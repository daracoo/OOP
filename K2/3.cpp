...
public:
    Demonstrator(int indeks, int *ocenki, int brojOcenki, char *imeIPrezime, Kurs *kursevi, int brojKursevi,
                 int brojCasovi) : Student(indeks, ocenki, brojOcenki), Predavach(imeIPrezime, kursevi, brojKursevi)
    {
    ...
    for (int i = 0; i < n; ++i)
        if (dynamic_cast<Demonstrator *>(studenti[i]))
            for (int j = 0; j < dynamic_cast<Demonstrator *>(studenti[i])->getBrojKursevi(); j++)
                if (!strcmp(kurs, dynamic_cast<Demonstrator *>(studenti[i])->operator[](j).getIme()))
                {
                    studenti[i]->pecati();
                    cout << endl;
                    break;
